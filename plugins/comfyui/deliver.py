...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--node", required=True, help="SaveImage node id to read outputs from")
    ...
    try:
        ...
        send(args.send_bin, args.chat_id, f"그림 만들다 실패했어: {exc}"[:400])
        return 1
    ...
    if entry is None:
        send(args.send_bin, args.chat_id,
             f"그림이 {int(args.timeout)}초 안에 안 끝나서 포기했어 (prompt_id={args.prompt_id})")
        return 1
    ...
    try:
        ...
        send(args.send_bin, args.chat_id, f"그림은 됐는데 파일을 못 옮겼어: {exc}"[:400])
        return 1

    send(args.send_bin, args.chat_id, f"[[image: {handed}]]\n{args.caption}")
    return 0
