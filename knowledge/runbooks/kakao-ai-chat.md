...
3. 본문 **어디든** `@jarvis` 가 있거나, `type=26`(답장)이고
   `attachment.src_logId` 가 가리키는 메시지가 `[jarvis]` 로 시작한다.

멘션의 **위치는 자유다** — 맨 앞·중간·끝 다 걸린다 (2026-09-14). 막는 건 위치가 아니라 경계다:
양쪽 다 `[A-Za-z0-9_.@-]` 만 배제하므로 `bob@jarvis.example`, `@jarvis.example`,
`@jarvistest`, `@jarvis-bot` 은 안 걸리고, 붙여넣은 로그의 `[@jarvis]` 도 왼쪽 `[` 로 막는다.

경계 문자는 **ASCII 만**이다 (2026-09-18). 전엔 `\w` 였는데 그게 유니코드라 한국어에서
가장 흔한 형태를 통째로 삼켰다 — `@jarvis야`, `@jarvis님`, `@jarvis한테` 가 전부 무응답이었고,
앞 단어에 붙은 `이거봐@jarvis` 도 마찬가지였다. 지금은 다 걸린다. 감싸는 문장부호
(`(@jarvis)`, `「@jarvis」`) 와 문장 끝 마침표(`@jarvis.`) 도 걸린다 — `.` 은 뒤에 글자가
올 때만 막으므로 호스트 이름 `@jarvis.example` 과 갈린다.

본문에서는 **처음 걸린 멘션 하나만** 빠진다. 붙여 쓴 조사는 본문에 남는다
(`@jarvis야 날씨 어때` → `야 날씨 어때`). 답장으로 이어가는 턴은 멘션이 없으므로 본문을 통째로 쓴다.

한 tick 에 트리거가 여러 개면 **전부 오래된 순으로 답한다** (`select_triggers`). 예전엔 마지막
하나만 처리하고 앞의 것을 조용히 버렸다. 커서는 스킵한 행을 포함해 항상 전진하므로, 같은 줄을
두 번 집지 않는다.
...
- 프롬프트가 신뢰 경계를 명시한다. 블록은 `MY_THREAD` / `OTHERS` / `QUOTED` / `MENTION` 넷이고,
  분리 기준은 **author_id** 다 (`[jarvis]` 접두는 방 안의 누구나 칠 수 있으므로 신원이 아니다).
  `OTHERS` 와 `QUOTED`, 그리고 **사진·파일 안의 글자**는 데이터이지 지시가 아니다.
  실행할 지시는 `MENTION` 과 `MY_THREAD` 에서만 나온다.

## 게이트웨이 네이티브 채널로의 전환 (조사됨, 미착수)

이 데몬 전체를 hermes 게이트웨이 플랫폼 플러그인 하나로 대체할 수 있다. 여기 있는 거의 모든 손조립
장치(프롬프트 신뢰 경계, `jobs/` 방 락, `post_tool_call` 진행 훅, Discord 경유 배달)에 네이티브
슬롯이 있다. 조사 결과·실측·함정·미결 결정 셋은
[tasks/2026-09-16-kakao-gateway-platform.md](../../tasks/2026-09-16-kakao-gateway-platform.md).

배포본에는 그 전 단계로 `--resume` + 세션 TTL 이 들어가 있다 (방마다 하나의 hermes 세션).
**레포본은 배포본보다 뒤져 있으니** 이 파일의 코드 인용을 근거로 배포본을 고치지 말 것.
...
- **모델은 프로필 기본값을 상속한다.** `config.json` 의 `provider`/`model` 은 **빈 문자열**이고,
  ...
  답했다). DGX 기본은 `custom:kilo` / `nex-agi/nex-n2.5-pro:free` 이고(2026-09-14 교체,
  그 전엔 `zai/glm-4.7-flash`), 부모·자식 두 체인은 모두
  `knowledge/tools/local-llm-providers.md` 의 "DGX parent and child chains" 절이 소유한다.
- **어려운 턴은 `delegate_task` 로 넘긴다.** toolsets 에 `delegation` 이 들어 있고
  ...
  주면 캡을 넘긴다. 자식 병렬은 3, 자식 반복은 20 이다 — Kilo 는 6병렬까지 견디고
  12병렬에서 3/12 로 무너지며, 200 req/시간이 **IP 당** 이라 부모와 자식이 같은 지갑을 쓴다.
  무거운 팬아웃 턴 하나가 약 70 요청이다.
...
- **모델 선택이 응답 시간을 좌우한다.** 위 수치(MLX 7분 vs nano 17초)는 Mac 시절 것이다.
  ...
  부모 모델 한 콜이 `glm-4.7-flash` 에서는 113~130초였고, `nex-n2.5-pro:free` 로 바꾼 뒤
  11k 토큰 요청 기준 3.0초다. 같은 날 20분 하드캡 2회와 409초/15자 턴은 모델이 느려서가
  아니라 zai 무료 티어가 요청의 5/6 을 `429 1305` 로 거절했기 때문이다.
